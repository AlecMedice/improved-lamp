...
import { WORLD, PLAYER, CAVE } from "@shared/config";
import { CAVES } from "@shared/world/caves";
...
// Cave entrances (CAVES) and the deterministic world model now come from the shared module,
// so the server validates movement against the exact same forest the clients render.
...
export class ForestRoom extends Room<GameState> {
  ...
  private applyMove(sid: string, p: Player, data: any) {
    ...
    const resolved = this.world.resolveCollision(rx, rz, PLAYER.radius);
    p.x = resolved.x;
    p.z = resolved.z;
