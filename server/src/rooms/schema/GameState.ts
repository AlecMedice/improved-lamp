...
/** One connected player — a Searcher (hunter) or Yeti. */
export class Player extends Schema {
  @type("string") role = "searcher"; // "searcher" | "yeti"
  @type("string") name = "Searcher";

  // Character specialty, dealt at match start (searchers only; "" = Yeti/unassigned). See
  ...
  @type("boolean") dazzled = false; // Yeti only: a searcher's sustained flashlight is blinding it (roar/grab locked)
...
/**
 * A trace Yeti leaves behind — the hint framework hunters follow.
 * Spawned server-side as Yeti moves; expires after a lifetime so the trail goes cold.
 */
...
export class GameState extends Schema {
  ...
  @type("number") totalNights = 3; // Yeti wins by surviving this many nights

  // Win condition: hunters must capture this many solid videos of Yeti (team total).
  ...
  @type("number") yetiSpeedMul = 1; // Yeti moves this much faster on later nights
  ...
  @type("number") roarCooldownSec = 25; // effective roar cooldown (for the Yeti client's UI gate)

  @type("string") winner = ""; // "" | "hunters" | "yeti"
}
