...
[`docs/UNITY_NOTES.md`](../docs/UNITY_NOTES.md) — **read that before changing anything here.**
...
`UNITY_NOTES.md` [fishnet-identity], which is the single most expensive trap in this port).
...
`Library/ScriptAssemblies` — see `UNITY_NOTES.md` [workflow]; it has caught real errors repeatedly.
...
- **Movement is interim owner-simulated** (client-authoritative NetworkTransform). Fine for
  ...
  task (`UNITY_NOTES.md` [open-items]). **Outcomes are already host-authoritative** — status, filming,
...
- **START NEW GAME throws a NullReferenceException** — almost certainly a scene/prefab identity
  ...
  `UNITY_NOTES.md` [fishnet-identity] has the full explanation.
...
- **Choppy framerate** — the browser build capped its resolution and Unity doesn't. Lower
  **resolution scale** in Settings or the pause menu; see `UNITY_NOTES.md` [perf] for what to cut next.
