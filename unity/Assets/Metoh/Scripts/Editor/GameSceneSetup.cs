// One-click wiring for the GAME scene (the ported Metoh slice) — the successor to the
// R1 cube scene. Builds Assets/Scenes/Mountain.unity: NetworkManager + Tugboat + connect HUD,
...
// (HPPlayer, ClueMarker). Requires the Metoh.Sim sources under Assets/ (the sync step
// copies them from the repo's csharp/Metoh.Sim).
//
//     Metoh → Set Up Game Scene (Mountain)     then Play → Host → START MATCH.
//     Metoh → Build Windows (Game)           → Build/Windows/Metoh.exe
using System.IO;
using System.Linq;
...
namespace Metoh.EditorTools
{
    public static class GameSceneSetup
    {
        ...
        private const string PrefabDir = "Assets/Metoh/Prefabs";
        ...
        private const string BuildPath = "Build/Windows/Metoh.exe";

        [MenuItem("Metoh/Set Up Game Scene (Mountain)")]
...
SetInputHandlingToBoth();
            ...
            // reach an already-built Mountain.unity. Anything tunable here has to be set here too.
...
AssignDefaultPrefabObjects(nm);
            ...
            Debug.Log("[GameSceneSetup] Mountain scene wired → " + ScenePath + ". Play → 'Host (server + client)' " +
                      "→ START MATCH. (If input is dead in Play mode, restart the editor once.)");
        }

        [MenuItem("Metoh/Build Windows (Game)")]
