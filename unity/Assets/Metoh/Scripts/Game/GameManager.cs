// The authoritative match loop — MountainRoom.ts reborn as a FishNet host system. Runs ONLY on
...
//
// Tunables are verbatim from server/src/rooms/MountainRoom.ts (the web build's source of truth).
...
namespace Metoh.Game
{
    public class GameManager : NetworkBehaviour
    {
        ...
        /// so changing the field default alone would never reach an already-built Mountain.unity.
        /// </summary>
        ...
        /// <summary>Wren drops a team-visible trail marker at her feet (MountainRoom's `mark` handler).</summary>
        ...
        /// <summary>
        /// A hunter drops a stakeout ping (MountainRoom's `ping` handler): one live ping per hunter —
        ...
        /// <summary>
        /// Yeti fast-travels between cave mouths (MountainRoom's `caveTravel` handler): it must be
        ...
        private bool ConeVisible(HPPlayer from, HPPlayer to, double range, double aimCos)
        {
            ...
            return !Metoh.Sim.Collision.LineBlocked(_world.Colliders,
