...
type Build = {
  ...
  hunch: number; // forward pitch of the upper body (Yeti leans; hunters stand)
};
...
export class RemotePlayer {
  ...
  private senseSilhouette?: THREE.Group; // depthTest-off glow so the local Yeti sees prey through trees
  ...
  private armBase = 0; // resting forward pitch of the arms (Yeti's hang forward)
  ...
  constructor(private scene: THREE.Scene, role: string, private audio?: AudioEngine) {
    ...
    this.stride = this.isYeti ? 2.3 : 1.7; // Yeti's gait is longer/heavier
    const b = this.isYeti ? YETI_BUILD : HUNTER_BUILD;
    ...
    if (this.isYeti) {
      // Faint eye-shine so hunters can spot Yeti in the dark (and then film it). Rides the head.
      ...
      // Hunter's recording tally light (hidden unless filming) — tension for Yeti.
      ...
      // Yeti's predator vision reveals this hunter through trees. Hidden until senses are on.
   ...
   * "upper body" group (leant forward for Yeti). Smooth-shaded capsules/spheres keep the
   * low-poly-but-not-blocky house style. Limbs pivot at the top so a rotation swings them.
   */
  private buildRig(b: Build) {
    const bf = this.isYeti;
    // Palettes: hunters read as a person (jacket/pants/skin/boots); Yeti is fur with a dark face.
    ...
    // Upper body (torso + head + arms) — leant forward by `hunch` for Yeti.
    ...
    // Arms hang from the shoulders (Yeti's are long and rest forward).
  ...
  /** Local Yeti's senses overlay: reveal this hunter's silhouette through the environment. */
  ...
  private animate(dt: number) {
    this.idleT += dt;
    const walk = Math.min(1, this.speed / (this.isYeti ? 3.5 : 3.0)); // 0 idle → 1 at cruising pace
  ...
  private tickFootsteps() {
    ...
    if (this.stepDist >= this.stride) {
      this.stepDist = 0;
      this.audio.playAt(this.isYeti ? "footstep_heavy" : "footstep_soft", x, z, {
        volume: (this.isYeti ? 0.7 : 0.4) * this.footstepVol,
        refDistance: 7,
      });
    }
  }
