...
- ✅ **Server‑authoritative movement** — shared deterministic world (`shared/world/WorldModel.ts`) gives the server terrain + collision; the `move` handler validates every update (world‑bounds clamp, **max‑speed gate** vs teleport/speedhack, **collision pushout** vs phasing, terrain feet‑clamp). Cave fast‑travel is a validated `caveTravel` command, not a client self‑teleport.
...
- ✅ **Shared deterministic world** — `shared/` (rng, world constants, **seeded `CAVES`**, `WorldModel`) imported by both client + server via the `@shared/*` alias; the old `Math.random()` cave duplication is gone (client + server now agree).
